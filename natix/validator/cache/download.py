...
def download_files(urls: List[str], output_dir: Union[str, Path], chunk_size: int = 8192) -> List[Path]:
    """
...
def openvid1m_err_handler(
    base_zip_url: str, output_path: Path, part_index: int, chunk_size: int = 8192, timeout: int = 300
) -> Optional[Path]:
    """
    ...
    """
    part_urls = [f"{base_zip_url}{part_index}_partaa", f"{base_zip_url}{part_index}_partab"]
    ...
    for part_url in part_urls:
        ...
        try:
            ...
            if response.status_code != 200:
                raise RequestException(f"HTTP {response.status_code}: {response.reason}")
